...
import {
    ...
    R_B_FL,
    B_L_FL,
    T_R_FL, T_L_FL, R_L_FL, T_B_FL, B_FL, T_FL, R_FL, L_FL, DEBUG
} from './constants';
...
export class TileRenderer {
    ...
    constructor(ctx: CanvasRenderingContext2D, tileAssets: TileAssets, tileMap: TileMap) {
        this.ctx = ctx;
    ...
    drawMap(): void {
        ...
        for (let y: number = 0; y < MAP_HEIGHT; y++) {
            for (let x: number = 0; x < MAP_WIDTH; x++) {
                const tile: number = this.tileMap.map[y][x].type;
